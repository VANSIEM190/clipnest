...
const ResponseList = ({ messageId }) => {
  ...
  return <>
    <div className=" w-2/4 mt-4 max-sm:w-full border-t pt-3 space-y-4">
      ...
      <div className="space-y-2">
        {paginatedMessages.map((response) => (
          <div key={response.id} className={`rounded-2xl p-2 sm:p-5 text-sm transition-all duration-300
          ${
            isDarkMode
      ...
      <div className="gap-2">
        <button
          ...
          className={`px-2 py-1 sm:px-3 sm:py-1.5 text-[10px] sm:text-sm bg-blue-500 mr-1
            ...
            ${isDarkMode ? "bg-gradient-to-br from-gray-800/30 via-gray-900/90 to-black/90 text-gray-100" : "bg-gray-300 text-gray-900 hover:bg-gray-400"}`}
            disabled={!isVisible}
        >
          <FiSend />
        </button>
